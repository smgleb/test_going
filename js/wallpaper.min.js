...
let addOrDelSectionBlock = (e, addOrDel) => {
	wallpaperForm = document.forms.wallpapercalc;
				...
				<div class="inp-item__body">
					...
					<div class="inp-item__inp-row inp-item__inp-row_num">
						...
						<input class="inp-item__input inp-item__input_num" id="window_${sectionItemLength}-num"
							...
							data-value="" placeholder="1" value="1">
...
let InpCorrect = () => {
	...
	for (let i = 0; i < InpWindow.length; i++) {
		if (InpWindow[i].value !== '' && InpWindow[i].value !== '0') {
			arrTrue.push(true)
		} else {
			arrTrue.push(false)
	...
	for (let i = 0; i < InpDoor.length; i++) {
		if (InpDoor[i].value !== '' && InpDoor[i].value !== '0') {
			arrTrue.push(true)
		} else {
			arrTrue.push(false)
...
let submitRes = () => {
	...
	if (roomBlock.classList.contains('_active')) {
		...
		for (let i = 0; i < InpRoom.length; i++) {
			if (InpRoom[i].value !== '' && InpRoom[i].value !== '0') {
				arrTrue.push(true)
			} else {
				arrTrue.push(false)
	...
	if (wallBlock.classList.contains('_active')) {
		...
		for (let i = 0; i < InpWall.length; i++) {
			if (InpWall[i].value !== '' && InpWall[i].value !== '0') {
				arrTrue.push(true)
			} else {
				arrTrue.push(false)
...
if (localStorage.getItem('wallItem')) {
	let roomItem = document.querySelector('#wall-items');
	roomItem.innerHTML = localStorage.getItem('wallItem');
}
if (localStorage.getItem('windowItem')) {
	let roomItem = document.querySelector('#window-items');
	roomItem.innerHTML = localStorage.getItem('windowItem');
}
if (localStorage.getItem('doorItem')) {
	let roomItem = document.querySelector('#door-items');
	roomItem.innerHTML = localStorage.getItem('doorItem');
}
// ===============
let getId = (id, input) => {
	...
	for (let id of ids) {
		...
		if (localStorage.getItem(id)) {
			if (input.type === 'number') {
				...
				// радио кнопки
				if (id === 'custom_rulon-width' && input.value !== '' && input.value !== '0') {
				...
				if (id === 'custom_rulon-length' && input.value !== '' && input.value !== '0') {
				...
				if (id === 'custom_stock' && input.value !== '' && input.value !== '0') {
