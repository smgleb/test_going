...
let resultFinalBlock = (res) => {
	...
	let resultDoorHeigth = null;
	let resultWindowHeigth = null;
...
<<<<<<< HEAD
	let resultFinalWallapersStepOne = ((perimetrOne / resultWidthNum)) / (Math.round(resultLengthNum / heightAllItem));
	let resultFinalWallapersStepWindow = (perimetrTwo / resultWidthNum) / (Math.round(resultLengthNum / (heightAllItem - resultHeightWindow)));
	let resultFinalWallapersStepDoor = (perimetrThree / resultWidthNum) / (Math.round(resultLengthNum / (heightAllItem - resultHeightDoor)));
=======

	let resultFinalWallapersStepOne = (Math.ceil(perimetrOne / resultWidthNum) + numDoorAndWindow) / Math.round(resultLengthNum / heightAllItem);
	let resultFinalWallapersStepWindow = (Math.ceil(perimetrTwo / resultWidthNum)) / Math.round(resultLengthNum / (heightAllItem - resultHeightWindow));
	let resultFinalWallapersStepDoor = (Math.ceil(perimetrThree / resultWidthNum)) / Math.round(resultLengthNum / (heightAllItem - resultHeightDoor));
>>>>>>> dbdead72aa0a34b5dc57424e2ec46d3225cc1d25
	...
	// console.log(resultFinalWallapers)
	// console.log(Math.ceil(perimetrOne / resultWidthNum) + numDoorAndWindow)
	// console.log((Math.round(resultLengthNum / heightAllItem)))
	console.log(resultFinalWallapersStepOne)
	console.log(resultFinalWallapersStepWindow)
	console.log(resultFinalWallapersStepDoor)
	// console.log(Math.round(resultLengthNum / heightAllItem))
...
let getId = (id, input) => {
	input.addEventListener("change", () => {
		if (input.type === 'number' && input.value !== '') {
			localStorage.setItem(id, input.value.replace(/[^0-9.,]/g, '').replace(/^0+/g, ''));
		}
		if (input.type === "radio") {
...
document.addEventListener('focusout', (e) => {
	...
	if (e.target.closest('.inp-item__input')) {
		...
		e.target.value = inpValue.replace(/[^0-9.,]/g, '').replace(/^0+/g, '');
		if (e.target.value !== '') {
		...
		if (resArr !== false) {
			...
			resultContainer.insertAdjacentHTML(
				...
				<div class="body-result__block">
					<div class="body-result__num-rulon">${resultFinalWallapers} трубок</div>
